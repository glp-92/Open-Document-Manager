...
export function DocumentsPanel({ documents, onUpload, lastCitedDocs = [] }: Props) {
  ...
  return (
    <aside className="w-[300px] min-w-[300px] h-screen flex flex-col border-l border-border bg-background">
      ...
      <div className="h-14 border-b border-border flex items-center px-4">
        ...
        <span className="ml-auto text-[11px] text-muted-foreground">{documents.length} files</span>
      </div>
      ...
      <div className="p-4">
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors cursor-pointer ${
            isDragOver ? 'border-accent bg-accent/5' : 'border-border hover:border-muted-foreground/30'
          }`}
          onClick={() => document.getElementById('file-upload')?.click()}
        >
          <Upload className={`w-5 h-5 mx-auto mb-2 ${isDragOver ? 'text-accent' : 'text-muted-foreground'}`} />
          <p className="text-sm font-medium text-foreground">Upload Documents</p>
          <p className="text-xs text-muted-foreground mt-1">Drop files here or click to browse</p>
          <input
            id="file-upload"
            type="file"
            multiple
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-1">
        <AnimatePresence initial={false}>
          {documents.map(doc => (
            ...
            >
              <DocumentItem doc={doc} isCited={lastCitedDocs.includes(doc.name)} />
            </motion.div>
          ))}
        </AnimatePresence>
      ...
      {lastCitedDocs.length > 0 && (
        <div className="border-t border-border p-4">
          ...
          {lastCitedDocs.map((name, i) => (
            <div key={i} className="flex items-center gap-2 text-xs text-foreground py-1">
