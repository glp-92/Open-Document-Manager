...
class ChatRouter:
    ...
    def _register_routes(self):
        ...
        async def find_chats_with_filters_pageable(
            filters: Annotated[ChatFilters, Query()], sql_session: Session = Depends(get_db)
        ):
            try:
                return self.chat_service.find_chats_with_filters_pageable(session=sql_session, filters=filters)
            except (ValidationError, Exception):
                traceback.print_exc()
                raise HTTPException(status_code=400, detail="bad request")

        @self.router.post("", status_code=201, response_model=ChatResponse)
        async def create_chat(new_chat_request: NewChatRequest, sql_session: Session = Depends(get_db)):
            try:
                return self.chat_service.create_chat(session=sql_session, new_chat_request=new_chat_request)
            except (ValidationError, Exception):
                traceback.print_exc()
                raise HTTPException(status_code=400, detail="bad request")

        @self.router.delete("/{chat_id}", status_code=204)
        async def delete_chat(chat_id: UUID, sql_session: Session = Depends(get_db)):
            try:
                return self.chat_service.delete_chat_by_id(session=sql_session, chat_id=chat_id)
